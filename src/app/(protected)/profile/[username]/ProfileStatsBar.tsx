...
export default function ProfileStatsBar({
  ...
  return (
    <div
      className="grid grid-cols-3 gap-2 md:gap-4 min-h-[72px] md:min-h-[80px] items-center rounded-[20px] px-4 md:px-6 py-4"
      style={{
    ...
    >
      {stats.map(({ label, value }) => (
        <div
          key={label}
          className="flex flex-col justify-center items-center min-h-[52px] rounded-[12px] touch-manipulation"
          role="group"
        ...
        >
          ...
          <span className="text-[12px] mt-0.5" style={{ color: '#9ba7b8' }}>
            {label}
          </span>
        </div>
      ))}
      <div className="col-span-3 flex items-center justify-center pt-3 mt-1 border-t border-[rgba(255,255,255,0.06)]">
        <TalentScoreBadge
